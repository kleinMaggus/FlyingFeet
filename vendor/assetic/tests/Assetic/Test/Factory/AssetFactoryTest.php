...
class AssetFactoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCreateAssetReference() {
		...
		$this->am->expects($this->any())->method('get')->with('jquery')->will($this->returnValue($referenced));
	...
	public function getHttpUrls() {
		return array(array('http://example.com/foo.css'), array('https://example.com/foo.css'), array('//example.com/foo.css'),);
	}
	...
	public function testFilter() {
		$this->fm->expects($this->once())->method('get')->with('foo')->will($this->returnValue($this->getMock('Assetic\\Filter\\FilterInterface')));
	...
	public function testInvalidFilter() {
		...
		$this->fm->expects($this->once())->method('get')->with('foo')->will($this->throwException(new \InvalidArgumentException()));
	...
	public function testWorkers() {
		...
		$worker->expects($this->exactly(3))->method('process')->with($this->isInstanceOf('Assetic\\Asset\\AssetInterface'));
	...
	public function testNestedFormula() {
		...
		$inputs = array('css/main.css', array(
		// nested formula
