...
class AsseticControllerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testRenderNotFound() {
		...
		$this->am->expects($this->once())->method('has')->with($name)->will($this->returnValue(false));
	...
	public function testRenderLastModifiedStale() {
		...
		$asset->expects($this->once())->method('dump')->will($this->returnValue($content));
	...
	public function testRenderETagFresh() {
		...
		$this->am->expects($this->once())->method('hasFormula')->with($name)->will($this->returnValue(true));
		$this->am->expects($this->once())->method('getFormula')->with($name)->will($this->returnValue($formula));
		$this->request->expects($this->once())->method('getETags')->will($this->returnValue(array('"' . $etag . '"')));
		$asset->expects($this->never())->method('dump');
	...
	public function testRenderETagStale() {
		...
		$this->am->expects($this->once())->method('hasFormula')->with($name)->will($this->returnValue(true));
		$this->am->expects($this->once())->method('getFormula')->with($name)->will($this->returnValue($formula));
		$this->request->expects($this->once())->method('getETags')->will($this->returnValue(array('"123"')));
		$asset->expects($this->once())->method('dump')->will($this->returnValue($content));
