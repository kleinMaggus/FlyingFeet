	...
	public function load(array $configs, ContainerBuilder $container) {
		...
		foreach ($container->getParameterBag()->resolveValue($config['bundles']) as $bundle) {
			...
			foreach (array('twig', 'php') as $engine) {
				$container
						->setDefinition('assetic.' . $engine . '_directory_resource.' . $bundle,
								new DirectoryResourceDefinition($bundle, $engine, array($container->getParameter('kernel.root_dir') . '/Resources/' . $bundle . '/views', dirname($rc->getFileName()) . '/Resources/views',)));
			}
		}
		foreach (array('twig', 'php') as $engine) {
