...
class ControllerListenerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testOnCoreController() {
		...
		$interceptor = $this->getMockBuilder('JMS\SecurityExtraBundle\Security\Authorization\Interception\MethodSecurityInterceptor')->disableOriginalConstructor()->getMock();

		$interceptor->expects($this->once())->method('invoke')->will($this->returnCallback(function () {
							return func_get_args();
						}));

		$container->expects($this->once())->method('get')->with($this->equalTo('security.access.method_interceptor'))->will($this->returnValue($interceptor));
		...
		$expected = array('roles' => array('ROLE_FOO'), 'run_as_roles' => array(), 'param_permissions' => array(), 'return_permissions' => array(),);
