...
class AfterInvocationManagerTest extends \PHPUnit_Framework_TestCase {
	public function testDecide() {
		...
		$provider1->expects($this->once())->method('decide')
				->with($this->isInstanceOf('Symfony\Component\Security\Core\Authentication\Token\TokenInterface'), $this->anything(), $this->equalTo($attributes), $this->equalTo('foo'))->will($this->returnValue('bar'));

		$provider2 = $this->getProvider();
		$provider2->expects($this->once())->method('decide')
				->with($this->isInstanceOf('Symfony\Component\Security\Core\Authentication\Token\TokenInterface'), $this->anything(), $this->equalTo($attributes), $this->equalTo('bar'))->will($this->returnValue('moo'));
	...
	public function testSupportsAttribute($attribute, $supported) {
		...
		$provider->expects($this->once())->method('supportsAttribute')->with($this->equalTo($attribute))->will($this->returnValue($supported));
	...
	public function testSupportsClass($class, $supported) {
		$provider = $this->getProvider();
		$provider->expects($this->once())->method('supportsClass')->with($this->equalTo($class))->will($this->returnValue($supported));
