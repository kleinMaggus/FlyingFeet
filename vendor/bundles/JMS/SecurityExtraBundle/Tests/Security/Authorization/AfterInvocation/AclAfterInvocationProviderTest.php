...
class AclAfterInvocationProviderTest extends \PHPUnit_Framework_TestCase {
	...
	public function testDecideDoesNotModifyReturnedObjectWhenNoObjectIdentityCanBeRetrieved() {
		...
		$oidStrategy->expects($this->once())->method('getObjectIdentity')->will($this->returnValue(null));

		$permissionMap = $this->getPermissionMap();
		$permissionMap->expects($this->once())->method('contains')->will($this->returnValue(true));
	...
	public function testDecideThrowsAccessDeniedExceptionWhenNoAclIsFound() {
		$oidStrategy = $this->getOidStrategy();
		$oidStrategy->expects($this->once())->method('getObjectIdentity')->will($this->returnValue(new ObjectIdentity(1, 'foo')));

		$sidStrategy = $this->getSidStrategy();
		$sidStrategy->expects($this->once())->method('getSecurityIdentities')->will($this->returnValue(array()));

		$permissionMap = $this->getPermissionMap();
		$permissionMap->expects($this->once())->method('contains')->will($this->returnValue(true));

		$aclProvider = $this->getAclProvider();
		$aclProvider->expects($this->once())->method('findAcl')->will($this->throwException(new AclNotFoundException('No ACL')));
	...
	public function testDecideThrowsAccessDeniedExceptionWhenNoAceIsFound() {
		...
		$permissionMap = $this->getPermissionMap();
		$permissionMap->expects($this->once())->method('contains')->will($this->returnValue(true));
		$permissionMap->expects($this->once())->method('getMasks')->will($this->returnValue($masks));

		$acl = $this->getMock('Symfony\Component\Security\Acl\Model\AclInterface');
		$acl->expects($this->once())->method('isGranted')->will($this->throwException(new NoAceFoundException('No ACE')));

		$aclProvider = $this->getAclProvider();
		$aclProvider->expects($this->once())->method('findAcl')->will($this->returnValue($acl));

		$oidStrategy = $this->getOidStrategy();
		$oidStrategy->expects($this->once())->method('getObjectIdentity')->will($this->returnValue(new ObjectIdentity(1, 'foo')));

		$sidStrategy = $this->getSidStrategy();
		$sidStrategy->expects($this->once())->method('getSecurityIdentities')->will($this->returnValue(array('foo')));
	...
	public function testDecide() {
		...
		$acl = $this->getMock('Symfony\Component\Security\Acl\Model\AclInterface');
		$acl->expects($this->once())->method('isGranted')->will($this->returnValue(true));
	...
	public function testSupportsAttribute() {
		...
		$permissionMap->expects($this->at(0))->method('contains')->with($this->equalTo('foo'))->will($this->returnValue(true));
		$permissionMap->expects($this->at(1))->method('contains')->with($this->equalTo('asdf'))->wilL($this->returnValue(false));
