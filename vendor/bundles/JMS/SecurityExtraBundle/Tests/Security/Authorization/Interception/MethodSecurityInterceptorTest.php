...
class MethodSecurityInterceptorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testInvokeAuthenticatesTokenIfItIsNotYetAuthenticated() {
		...
		$authManager->expects($this->once())->method('authenticate')->will($this->throwException(new AuthenticationException('Could not authenticate.')));
	...
	public function testInvokeCallsADMForRolesAndThrowsExceptionWhenInsufficientPriviledges() {
		...
		$context->expects($this->once())->method('setToken')->with($this->equalTo($token));

		$authManager->expects($this->once())->method('authenticate')->will($this->returnValue($token));

		$method = $this->getInvocation();
		$adm->expects($this->once())->method('decide')->with($this->equalTo($token), $this->equalTo(array('ROLE_FOO')), $this->equalTo($method))->will($this->returnValue(false));
	...
	public function testInvokeCallsADMForEachParamPermissionsAndThrowsExceptionOnInsufficientPermissions() {
		...
		$context->expects($this->once())->method('getToken')->will($this->returnValue($token = $this->getToken()));

		$metadata = $this->getMetadata();
		$metadata['param_permissions'] = array($p0 = array('ROLE_FOO', 'ROLE_ASDF'), $p1 = array('ROLE_MOO'),);
		$method = $this->getInvocation();
		$adm->expects($this->at(0))->method('decide')->with($this->equalTo($token), $this->equalTo($p0), $this->equalTo(new \stdClass()))->will($this->returnValue(true));
		$adm->expects($this->at(1))->method('decide')->with($this->equalTo($token), $this->equalTo($p1), $this->equalTo(new \stdClass()))->will($this->returnValue(false));
	...
	public function testInvokehandlesExceptionsFromWithintheInvokedMethodGracefully() {
		...
		$runAsManager->expects($this->once())->method('buildRunAs')->will($this->returnValue($runAsToken));
	...
	protected function getToken($isAuthenticated = true) {
		$token = $this->getMock('Symfony\Component\Security\Core\Authentication\Token\TokenInterface');
		$token->expects($this->once())->method('isAuthenticated')->will($this->returnValue($isAuthenticated));

		return $token;
	}

	protected function getMetadata(array $roles = array(), array $runAsRoles = array(), array $paramPermissions = array(), array $returnPermissions = array()) {
		return array('roles' => $roles, 'run_as_roles' => $runAsRoles, 'param_permissions' => $paramPermissions, 'return_permissions' => $returnPermissions,);
	}
	...
	protected function getInterceptor() {
		...
		return array(new MethodSecurityInterceptor($securityContext, $authenticationManager, $accessDecisionManager, $afterInvocationManager, $runAsManager), $securityContext, $authenticationManager, $accessDecisionManager, $afterInvocationManager, $runAsManager,);
	}
