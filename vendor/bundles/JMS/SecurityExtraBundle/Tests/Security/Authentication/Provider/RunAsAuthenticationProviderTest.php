...
class RunAsAuthenticationProviderTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAuthenticateThrowsExceptionWhenKeysDontMatch() {
		...
		$token->expects($this->once())->method('getKey')->will($this->returnValue('moo'));
	...
	public function testAuthenticate() {
		$provider = new RunAsAuthenticationProvider('foo');
		$token = $this->getSupportedToken();
		$token->expects($this->once())->method('getKey')->will($this->returnValue('foo'));
	...
	protected function getSupportedToken() {
		return $this->getMockBuilder('JMS\SecurityExtraBundle\Security\Authentication\Token\RunAsUserToken')->disableOriginalConstructor()->getMock();
	}
}
