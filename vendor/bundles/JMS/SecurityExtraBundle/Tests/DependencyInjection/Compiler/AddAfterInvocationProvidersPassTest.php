...
class AddAfterInvocationProvidersPassTest extends \PHPUnit_Framework_TestCase {
	public function testProcessStopsWhenNoAfterInvocationManager() {
		...
		$container->expects($this->once())->method('hasDefinition')->with($this->equalTo('security.access.after_invocation_manager'))->will($this->returnValue(false));
	...
	public function testProcessRemovesAclProviderIfAclIsNotActive() {
		...
		$container->register('security.access.after_invocation.acl_provider')->addTag('security.after_invocation.provider');
