...
class ServiceAnalyzer {
	...
	public function __construct($class, Reader $reader) {
		...
		$this->driver = new DriverChain(array(new AnnotationDriver($reader),));
		$this->analyzed = false;
	}
	...
	private function normalizeMetadata() {
		...
		foreach ($this->metadata->classMetadata as $class) {
			...
			foreach ($class->methodMetadata as $name => $method) {
				...
				} else if (false === $secureMethods[$name]->satisfiesParentSecurityPolicy && $method->reflection->getDeclaringClass()->getName() !== $secureMethods[$name]->reflection->getDeclaringClass()->getName()) {
					throw new \RuntimeException(
							sprintf('Unresolved security metadata conflict for method "%s::%s" in "%s". Please copy the respective annotations, and add @SatisfiesParentSecurityPolicy to the child method.', $secureMethods[$name]->reflection->getDeclaringClass()->getName(), $name,
									$secureMethods[$name]->reflection->getDeclaringClass()->getFileName()));
				}
			}
		}
		...
		while (true) {
			foreach ($rootClass->getMethods() as $method) {
				...
				if ($secureMethods[$name]->reflection->getDeclaringClass()->getName() !== $rootClass->getName()) {
					throw new \RuntimeException(
							sprintf('You have overridden a secured method "%s::%s" in "%s". ' . 'Please copy over the applicable security metadata, and ' . 'also add @SatisfiesParentSecurityPolicy.', $secureMethods[$name]->reflection->getDeclaringClass()->getName(), $name, $rootClass->getName()));
				}
