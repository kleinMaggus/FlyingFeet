...
class ProfilerController extends ContainerAware {
	...
	public function panelAction($token) {
		...
		return $this->container->get('templating')
				->renderResponse($this->getTemplateName($profiler, $panel),
						array('token' => $token, 'profile' => $profile, 'collector' => $profile->getCollector($panel), 'panel' => $panel, 'templates' => $this->getTemplates($profiler),));
	}
	...
	public function exportAction($token) {
		...
		return new Response($profiler->export($profile), 200, array('Content-Type' => 'text/plain', 'Content-Disposition' => 'attachment; filename= ' . $token . '.txt',));
	}
	...
	public function toolbarAction($token, $position = null) {
		...
		return $this->container->get('templating')
				->renderResponse('WebProfilerBundle:Profiler:toolbar.html.twig',
						array('position' => $position, 'profile' => $profile, 'templates' => $this->getTemplates($profiler), 'profiler_url' => $url, 'verbose' => $this->container->get('web_profiler.debug_toolbar')->isVerbose()));
	}
	...
	public function searchBarAction() {
		...
		return $this->container->get('templating')->renderResponse('WebProfilerBundle:Profiler:search.html.twig', array('token' => $token, 'ip' => $ip, 'url' => $url, 'limit' => $limit,));
	}
	...
	public function searchResultsAction($token) {
		...
		return $this->container->get('templating')
				->renderResponse('WebProfilerBundle:Profiler:results.html.twig',
						array('token' => $token, 'profile' => $profile, 'tokens' => $profiler->find($ip, $url, $limit), 'ip' => $ip, 'url' => $url, 'limit' => $limit, 'panel' => null,));
	}
	...
	public function searchAction() {
		...
		return new RedirectResponse($this->container->get('router')->generate('_profiler_search_results', array('token' => $tokens ? $tokens[0]['token'] : 'empty', 'ip' => $ip, 'url' => $url, 'limit' => $limit,)));
	}
