...
abstract class AbstractDoctrineExtension extends Extension {
	...
	protected function loadMappingInformation(array $objectManager, ContainerBuilder $container) {
		...
		foreach ($objectManager['mappings'] as $mappingName => $mappingConfig) {
			...
			$mappingConfig = array_replace(array('dir' => false, 'type' => false, 'prefix' => false,), (array) $mappingConfig);
	...
	protected function registerMappingDrivers($objectManager, ContainerBuilder $container) {
		...
		foreach ($this->drivers as $driverType => $driverPaths) {
			...
			if ($container->hasDefinition($mappingService)) {
				...
				$mappingDriverDef = new Definition('%' . $this->getObjectManagerElementName('metadata.' . $driverType . '.class%'), array(new Reference($this->getObjectManagerElementName('metadata.annotation_reader')), array_values($driverPaths)));
			} else {
				$mappingDriverDef = new Definition('%' . $this->getObjectManagerElementName('metadata.' . $driverType . '.class%'), array(array_values($driverPaths)));
			}
	...
	protected function assertValidMappingConfiguration(array $mappingConfig, $objectManagerName) {
		...
		if (!in_array($mappingConfig['type'], array('xml', 'yml', 'annotation', 'php', 'staticphp'))) {
			throw new \InvalidArgumentException(
					sprintf(
							'Can only configure "xml", "yml", "annotation", "php" or ' . '"staticphp" through the DoctrineBundle. Use your own bundle to configure other metadata drivers. ' . 'You can register them by adding a a new driver to the ' . '"%s" service definition.',
							$this->getObjectManagerElementName($objectManagerName . '.metadata_driver')));
		}
	}
