...
class MonologExtension extends Extension {
	...
	public function load(array $configs, ContainerBuilder $container) {
		...
		if (isset($config['handlers'])) {
			...
			$this
					->addClassesToCompile(
							array('Monolog\\Formatter\\FormatterInterface', 'Monolog\\Formatter\\LineFormatter', 'Monolog\\Handler\\HandlerInterface', 'Monolog\\Handler\\AbstractHandler', 'Monolog\\Handler\\AbstractProcessingHandler', 'Monolog\\Handler\\StreamHandler',
									'Monolog\\Handler\\FingersCrossedHandler', 'Monolog\\Logger', 'Symfony\\Bridge\\Monolog\\Logger', 'Symfony\\Bridge\\Monolog\\Handler\\DebugHandler',));
		}
	}
	...
	private function buildHandler(ContainerBuilder $container, $name, array $handler) {
		...
		case 'stream':
			$definition->setArguments(array($handler['path'], $handler['level'], $handler['bubble'],));
			break;
		...
		case 'rotating_file':
			$definition->setArguments(array($handler['path'], $handler['max_files'], $handler['level'], $handler['bubble'],));
			break;
		...
		case 'fingers_crossed':
			...
			$definition->setArguments(array(new Reference($nestedHandlerId), $handler['action_level'], $handler['buffer_size'], $handler['bubble'], $handler['stop_buffering'],));
			break;
		...
		case 'buffer':
			...
			$definition->setArguments(array(new Reference($nestedHandlerId), $handler['buffer_size'], $handler['level'], $handler['bubble'],));
			break;
		...
		case 'syslog':
			$definition->setArguments(array($handler['ident'], $handler['facility'], $handler['level'], $handler['bubble'],));
			break;
		...
		case 'swift_mailer':
			...
			$definition->setArguments(array(new Reference('mailer'), $prototype, $handler['level'], $handler['bubble'],));
			break;
