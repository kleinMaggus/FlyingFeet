...
class InfoDoctrineCommandTest extends TestCase {
	public function testAnnotationsBundle() {
		...
		$output->expects($this->at(0))->method('writeln')->with($this->equalTo('Found <info>1</info> entities mapped in entity manager <info>default</info>:'));
		$output->expects($this->at(1))->method('writeln')->with($this->equalTo('<info>[OK]</info>   Fixtures\Bundles\YamlBundle\Entity\Test'));
		...
		$kernel->expects($this->once())->method('getContainer')->will($this->returnValue($testContainer));
