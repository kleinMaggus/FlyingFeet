		...
		foreach ($dom->documentElement->childNodes as $node) {
			...
			if (!$this->container->hasExtension($node->namespaceURI)) {
				...
				throw new \InvalidArgumentException(
						sprintf('There is no extension able to load the configuration for "%s" (in %s). Looked for namespace "%s", found %s', $node->tagName, $file, $node->namespaceURI, $extensionNamespaces ? sprintf('"%s"', implode('", "', $extensionNamespaces)) : 'none'));
			}
		}
	}
